...
Original file is located at
    https://colab.research.google.com/drive/1hqv3iplJodfcw9H0NvnNTnU4FjR9TvK1
...
2. Distribusi dari release_year : Terdapat lonjakan signifikan pada tahun 2000 ke atas, terutama 2015–2020. Distribusi ini menunjukkan bahwa Netflix lebih banyak menambahkan konten modern dibanding konten lama. Skewed to the left (negative skew) hal ini dikarenakan mayoritas data berada di tahun-tahun baru, sedangkan data tahun lama sangat sedikit (misalnya 1925 hanya 1 data).
...
duration_df.columns = ['release_year', 'count']
grouped_duration = duration_df.groupby('count')['release_year'].apply(list).reset_index()
grouped_duration.columns = ['jumlah_tayangan', 'daftar_durasi']
grouped_duration.tail(10)

country = df['country'].value_counts()
country_df = country.reset_index()
country_df.columns = ['release_year', 'count']
grouped_country = country_df.groupby('count')['release_year'].apply(list).reset_index()
...
listed_in_df.columns = ['release_year', 'count']
grouped_listed_in = listed_in_df.groupby('count')['release_year'].apply(list).reset_index()
...
def get_recommendations(title, cosine_sim=cosine_sim):
    ...
    sim_scores = sim_scores[1:6]  # 5 teratas
    content_indices = [i[0] for i in sim_scores]
    return df[['title', 'listed_in', 'type', 'description']].iloc[content_indices]

"""## Evaluasi

#### Hasil Evaluasi Model
Baris kode ini menjalankan fungsi get_recommendations() untuk judul "Narcos", dan mengembalikan 5 konten lain yang paling mirip secara konten dengan "Narcos". ketika fungsi dijalankan maka sistem akan mengecek apakah judul terdapat di film yang ada di netflix, kemudian mengambil similiarity yang paling tinggi nilainya kemudian mengambil 5 konten yang nilainya paling tinggi.
